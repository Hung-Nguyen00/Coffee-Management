...
class UploadFileService:
    ...
    def delete(self, file_path: str, file_id: str = None):
        ...
        if not upload_file:
            return
        default_storage.delete(upload_file.file_path.path)
    ...
    def create_thumbnail(self, file_path: str, thumb_size: Tuple[int, int]):
        ...
        image = Image.open(file_abspath_path)
        image.thumbnail(thumb_size, Image.ANTIALIAS)
        file_path_name, file_ext = os.path.splitext(str(file_path))
        name_suffix = "x".join(map(str, thumb_size))
        output_path = file_path_name + "_" + name_suffix + ".png"
        image.save(settings.MEDIA_ROOT + "/" + str(output_path), "PNG")
        upload_file_thumbnail = upload_file.thumbnail_path
        if upload_file_thumbnail is None:
            upload_file_thumbnail = {name_suffix: output_path}
        else:
            upload_file_thumbnail.update({name_suffix: output_path})
